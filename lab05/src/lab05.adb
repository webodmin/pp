...
procedure lab05 is
  arraySize: constant Integer := 10000;
  maxTasksLab01: constant Integer := 200; -- number of threads for method 1
  ...
  protected Lab01Sum is
    entry set_sum(part: in Long_Long_Integer);
  ...
  protected body Lab01Sum is
    entry set_sum(part: in Long_Long_Integer) when True is
    begin
      grandTotal := grandTotal + part;
      taskCounter := taskCounter + 1;
    end set_sum;

    entry get_sum(sum: out Long_Long_Integer) when taskCounter = maxTasksLab01 is
    begin
      sum := Lab01Sum.grandTotal;
    end get_sum;
  end Lab01Sum;
  ...
  begin
    ...
    Lab01Sum.set_sum(part_sum(MyTask1.left, MyTask1.right));
  end MyTask1;
...
begin
  ...
  Lab01Sum.get_sum(result);
  Put_Line("FINAL RESULT after all tasks completed (lab01 method): " & result'img);
